...
int hal_can_init( void )
{
    ...
    //can_init.CAN_Mode = CAN_Mode_LoopBack;  // loop back mode
    // baudrate = PCLK1 / (SJW+BS1+BS2) / Prescaler
    //          = 36M / (1+3+5) / 4 = 1000kbps
...
//const can_baudrate_config_t baudrate[] = {
//{ 10000, 0, 0, 0, 0 },
//{ 20000, },
//    20k
//    40k
...
int hal_can_filter_set( int index, can_filter_t *filter, int enabled )
{
    ...
    if( filter->ext )
    {
        f |= ((filter->ext_id & 0x3FFFF)<<3) | 4;
        m |= ((filter->ext_id_mask & 0x3FFFF)<<3) |4;
    }
    if( filter->remote )
    {
        f |= 2;
...
int hal_can_filter_get( int index, can_filter_t *filter, int *enabled )
{
    ...
    if( CAN1->FA1R & (1<<index) )
    {
        *enabled = 1;
        ...
        if( filter->ext  )
        {
            filter->ext_id = (f >> 3) & 0x3FFFF;
            filter->ext_id_mask = (m >> 3) & 0x3FFFF;
        }
    }
    else
        *enabled = 0;
