 ...
 *
 * The number is blinked digital-by-digtal from left to right (up to 9 digits).
 ...
 *
 *
 * This task runs as an auxiliary service in a micro-kerenl RTOS environment,
 ...
 * 
 * To use this service task, copy or symbolic link task_blink.c/h files to your
 * application and call task_blink_init() before you use it.
 *
 * An independent shell command 'error' (alias name 'e') can
 * modify the number in runtime, and stop the task from blinking.
 *
 * As this maybe the only running task, it will periodically call hal_wdg_clear to
 * feed the watchdog.
 * Task priority is set OS_PRIORITY_NORMAL (same as shell task), modify it if
 * necessary.
 * 
 * The macro ERRNO_LED_NORMAL is set for errno zero, ERRNO_LED_ERROR for 
 * other non-zero number, normally they are both set to the same led (index 0).
 * You can change them to two different leds (use a RED led for warning/error
 * maybe a good ieda).
 *
