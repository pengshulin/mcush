...
static GPIO_InitTypeDef init;
uint8_t dat_output_mode=0;
...
void hal_lcd_port_set_input(void)
{
    if( dat_output_mode )
    {
...
uint16_t hal_lcd_port_readdata(void)
{
    uint16_t dat, vd, ve;
    vd = GPIO_ReadInputData(GPIOD);
    ve = GPIO_ReadInputData(GPIOE);
...
#define  LCD_READ()   hal_lcd_port_readdata()
#define  LCD_INPUT()  hal_lcd_port_set_input()
#define  LCD_INPUT()  hal_lcd_port_set_input()
...
uint16_t hal_lcd_read_data(void)
{
    uint16_t ret;
    LCD_CS_L;
    LCD_RS_H;
