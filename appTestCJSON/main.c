...
static int print_preallocated(cJSON *root)
{
    ...
    if (!cJSON_PrintPreallocated(root, buf, (int)len, 1)) {
        shell_printf("cJSON_PrintPreallocated failed!\n");
        if (strcmp(out, buf) != 0) {
            shell_printf("cJSON_PrintPreallocated not the same as cJSON_Print!\n");
            shell_printf("cJSON_Print result:\n%s\n", out);
            shell_printf("cJSON_PrintPreallocated result:\n%s\n", buf);
        }
        free(out);
        free(buf_fail);
        free(buf);
    ...
    if (cJSON_PrintPreallocated(root, buf_fail, (int)len_fail, 1)) {
        shell_printf("cJSON_PrintPreallocated failed to show error with insufficient memory!\n");
        shell_printf("cJSON_Print result:\n%s\n", out);
        shell_printf("cJSON_PrintPreallocated result:\n%s\n", buf_fail);
        free(out);
        free(buf_fail);
