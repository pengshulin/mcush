...
static pointer port_from_file(scheme *sc, int fin_handle, int prop);
...
static port *port_rep_from_file(scheme *sc, int fin_handle, int prop);
...
static void check_cell_alloced(pointer p, int expect_alloced)
{
  ...
  if(typeflag(p) & !expect_alloced)
    {
      shell_printf("Cell is already allocated!\n");
    }
  if(!(typeflag(p)) & expect_alloced)
    {
      shell_printf("Cell is not allocated!\n");
    }
...
static int file_interactive(scheme *sc) {
 return sc->file_i==0 && sc->load_stack[0].rep.stdio.file==0
...
static port *port_rep_from_file(scheme *sc, int f, int prop)
{
    port *pt;
...
static pointer port_from_file(scheme *sc, int f, int prop) {
  port *pt;
  pt=port_rep_from_file(sc,f,prop);
  if(pt==0) {
    return sc->NIL;
...
static int basic_inchar(port *pt) {
  if(pt->kind & port_file) {
    return mcush_getc(pt->rep.stdio.file);
  } else {
...
static void backchar(scheme *sc, int c) {
  ...
  pt=sc->inport->_object._port;
  if(pt->kind&port_file) {
    mcush_ungetc(pt->rep.stdio.file, c);
  } else {
...
INTERFACE void putstr(scheme *sc, const char *s) {
  ...
  if(pt->kind&port_file) {
    mcush_puts(pt->rep.stdio.file, s);
  } else {
    for(;*s;s++) {
...
static void putchars(scheme *sc, const char *s, int len) {
  port *pt=sc->outport->_object._port;
  if(pt->kind&port_file) {
    mcush_write(pt->rep.stdio.file, (void*)s, len);
  } else {
    for(;len;len--) {
      if(pt->rep.string.curr!=pt->rep.string.past_the_end) {
        *pt->rep.string.curr++=*s++;
...
INTERFACE void putcharacter(scheme *sc, int c) {
  port *pt=sc->outport->_object._port;
  if(pt->kind&port_file) {
    mcush_putc(pt->rep.stdio.file, c);
  } else {
    if(pt->rep.string.curr!=pt->rep.string.past_the_end) {
...
static pointer _Error_1(scheme *sc, const char *s, pointer a) {
     ...
     if (sc->load_stack[sc->file_i].kind & port_file &&
         sc->load_stack[sc->file_i].rep.stdio.file != 0) {
...
static pointer opexe_0(scheme *sc, enum scheme_opcodes op) {
     ...
     case OP_LOAD:       /* load */
          if(file_interactive(sc)) {
               mcush_printf(sc->outport->_object._port->rep.stdio.file,
...
void scheme_set_input_port_file(scheme *sc, int fin) {
...
void scheme_set_output_port_file(scheme *sc, int fout) {
...
void scheme_load_file(scheme *sc, int fin)
{ scheme_load_named_file(sc,fin,0); }

void scheme_load_named_file(scheme *sc, int fin, const char *filename) {
  dump_stack_reset(sc);
  sc->envir = sc->global_env;
  sc->file_i=0;
...
int tinyscheme_main(int argc, char **argv) {
  scheme sc;
  int fin;
  ...
  if(!scheme_init(&sc)) {
    shell_printf("Could not initialize!\n");
    return 2;
  }
  scheme_set_input_port_file(&sc, 0);
  scheme_set_output_port_file(&sc, 1);
#if USE_DL
  ...
  do {
    ...
    if(isfile && fin==0) {
      shell_printf("Could not open file %s\n",file_name);
    } else {
      if(isfile) {
      ...
      if(!isfile || fin!=0) {
        if(sc.retcode!=0) {
          shell_printf("Errors encountered reading %s\n",file_name);
        }
        if(isfile) {
          mcush_close(fin);
        }
