 ...
 */
    ...
    #define HAL_ADC_RCC_DMA_ENABLE_BIT     LL_AHB1_GRP1_PERIPH_DMA2
    ...
    #define HAL_ADCx_DATA_REG_ADDR         ((uint32_t)0x4001204C)
    #define HAL_DMAx                       DMA2
    #define HAL_DMA_Streamy                LL_DMA_STREAM_0
    #define HAL_DMA_Channelz               LL_DMA_CHANNEL_0
#endif

    
static uint8_t _adc_in_use;

static uint16_t _adc_value[HAL_ADC_NUM] __attribute__((section(".dma")));
...
void hal_adc_init( void )
{
    ...
    /* adc init */
    HAL_ADC_RCC_ADC_ENABLE_CMD(HAL_ADC_RCC_ADC_ENABLE_BIT);

    LL_ADC_CommonDeInit( ADC );
    adc_common_init.CommonClock = LL_ADC_CLOCK_SYNC_PCLK_DIV6;
    ...
    adc_reg_init.SequencerLength = LL_ADC_REG_SEQ_SCAN_ENABLE_8RANKS;
    ...
    {
        //LL_ADC_SetChannelSamplingTime( HAL_ADCx, __LL_ADC_DECIMAL_NB_TO_CHANNEL(i), LL_ADC_SAMPLINGTIME_15CYCLES);
        LL_ADC_SetChannelSamplingTime( HAL_ADCx, __LL_ADC_DECIMAL_NB_TO_CHANNEL(i), LL_ADC_SAMPLINGTIME_480CYCLES);
    ...
    LL_DMA_DeInit( HAL_DMAx, HAL_DMA_Streamy );
    ...
    LL_DMA_Init( HAL_DMAx, HAL_DMA_Streamy, &dma_init );
